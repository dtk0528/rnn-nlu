...
#     state = initial_state
            ...
            for i in xrange(num_heads):
                with variable_scope.variable_scope("Attention_%d" % i):
                    ...
                    if use_attention is False:  # apply mean pooling
                        weights = tf.tile(
                            sequence_length, tf.stack([attn_length]))
                        weights = array_ops.reshape(weights, tf.shape(s))
                        a = array_ops.ones(
                            tf.shape(s), dtype=dtype) / math_ops.to_float(weights)
        ...
        if initial_state_attention:
            attn_weights, attns = attention(
                initial_state, use_attention=use_attention)

        # with variable_scope.variable_scope(scope or "Linear"):
        matrix = variable_scope.get_variable(
            "Out_Matrix", [attn_size, output_size])
        # NOTE: here we temporarily assume num_head = 1
        res = math_ops.matmul(attns[0], matrix)
        bias_start = 0.0
