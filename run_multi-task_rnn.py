...
tf.app.flags.DEFINE_integer("word_embedding_size",
                            128, "Size of the word embedding")
    ...
    with tf.gfile.GFile(source_path, mode="r") as source_file:
        with tf.gfile.GFile(target_path, mode="r") as target_file:
            with tf.gfile.GFile(label_path, mode="r") as label_file:
                source, target, label = source_file.readline(
                ), target_file.readline(), label_file.readline()
                counter = 0
...
def train():
    ...
    with tf.Session() as sess:
        ...
        model, model_test = create_model(
            sess, len(vocab), len(tag_vocab), len(label_vocab))
        ...
        while model.global_step.eval() < FLAGS.max_training_steps:
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                def run_valid_test(data_set, mode):  # mode: Eval, Test
                    ...
                    for bucket_id in xrange(len(_buckets)):
                        ...
                        for i in xrange(len(data_set[bucket_id])):
                            ...
                            if task['tagging'] == 1:
                                word_list.append(
                                    [rev_vocab[x[0]] for x in encoder_inputs[:sequence_length[0]]])
                                ref_tag_list.append(
                                    [rev_tag_vocab[x[0]] for x in tags[:sequence_length[0]]])
                                hyp_tag_list.append(
                                    [rev_tag_vocab[np.argmax(x)] for x in tagging_logits[:sequence_length[0]]])
                    ...
                    if task['tagging'] == 1:
                        ...
                        tagging_eval_result = conlleval(
                            hyp_tag_list, ref_tag_list, word_list, taging_out_file)
                        print("  %s f1-score: %.2f" %
                ...
                if task['tagging'] == 1 and valid_tagging_result['f1'] > best_valid_score:
                    ...
                    subprocess.call(['mv', current_taging_valid_out_file,
